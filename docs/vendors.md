...
| **Rotation / exit plan** | Restore on different host: stand up new VPS, install Docker, paste BWS token, `task up` then SSH into the new box and run `docker exec -it infra-pg-backup /infra-pg-backup restore`. Switching cloud (DigitalOcean / OVH) is the same runbook — `IAC_BOOTSTRAP_HCLOUD_TOKEN` swaps with the provider's |
